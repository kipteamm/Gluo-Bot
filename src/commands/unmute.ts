...
export default new Command({
    ...
    async execute(i, [ member, reason ]) {
        ...
        if (!isTimedOut) {
            await i.editReply(`${member.user.username} is not timed out!`)
            return false
        }

        const timed = await member.disableCommunicationUntil(null, reason).catch(noop)
        if (!timed) {
            await i.editReply(`Failed to remove timeout from ${member.user.username}.`)
            return false
        }

        await i.editReply(`Successfully removed timeout from ${member.user.username}!`)
